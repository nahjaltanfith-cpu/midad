...
const fallbackValues = [
  { key: "v1", accent: "primary", icon: "M12 18v-5.25m0 0a6.01 6.01 0 001.5-.189m-1.5.189a6.01 6.01 0 01-1.5-.189m3.75 7.478a12.06 12.06 0 01-4.5 0m3.75 2.383a14.406 14.406 0 01-3 0M14.25 18v-.192c0-.983.658-1.823 1.508-2.316a7.5 7.5 0 10-7.517 0c.85.493 1.509 1.333 1.509 2.316V18" },
  { key: "v2", accent: "gold", icon: "M18 18.72a9.094 9.094 0 003.741-.479 3 3 0 00-4.682-2.72m.94 3.198l.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0112 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 016 18.719m12 0a5.971 5.971 0 00-.941-3.197m0 0A5.995 5.995 0 0012 12.75a5.995 5.995 0 00-5.058 2.772m0 0a3 3 0 00-4.681 2.72 8.986 8.986 0 003.74.477m.94-3.197a5.971 5.971 0 00-.94 3.197M15 6.75a3 3 0 11-6 0 3 3 0 016 0zm6 3a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0zm-13.5 0a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0z" },
  { key: "v3", accent: "primary", icon: "M3.75 3v11.25A2.25 2.25 0 006 16.5h2.25M3.75 3h-1.5m1.5 0h16.5m0 0h1.5m-1.5 0v11.25A2.25 2.25 0 0118 16.5h-2.25m-7.5 0h7.5m-7.5 0l-1 3m8.5-3 1 3m0 0 .5 1.5m-.5-1.5h-9.5m0 0-.5 1.5M9 11.25v1.5M12 9v3.75m3-6v6" },
  { key: "v4", accent: "gold", icon: "M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" },
];

export default function VisionSection() {
  const { t, lang } = useI18n();
  const { rows: dbValues } = useDbList<{ id: string; title_ar: string; title_en: string | null; desc_ar: string | null; desc_en: string | null; icon_path: string; accent: string }>("core_values");

  const coreValues = dbValues.length
    ? dbValues.map((v, i) => ({
        key: v.id,
        accent: v.accent || "primary",
        icon: v.icon_path || fallbackValues[i % fallbackValues.length].icon,
        title: lang === "ar" ? v.title_ar : v.title_en || v.title_ar,
        desc: lang === "ar" ? v.desc_ar || "" : v.desc_en || v.desc_ar || "",
      }))
    : fallbackValues.map((v) => ({
        ...v,
        title: t(`vision.${v.key}Title`),
        desc: t(`vision.${v.key}Desc`),
      }));
