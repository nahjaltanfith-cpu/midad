...
const fallbackInitiatives = [
  { icon: "M4.26 10.147a60.438 60.438 0 00-.491 6.347A48.62 48.62 0 0112 20.904a48.62 48.62 0 018.232-4.41 60.46 60.46 0 00-.491-6.347m-15.482 0a50.636 50.636 0 00-2.658-.813A59.906 59.906 0 0112 3.493a59.903 59.903 0 0110.399 5.84c-.896.248-1.783.52-2.658.814m-15.482 0A50.717 50.717 0 0112 13.489a50.702 50.702 0 017.74-3.342M6.75 15a.75.75 0 100-1.5.75.75 0 000 1.5zm0 0v-3.675A55.378 55.378 0 0112 8.443m-7.007 11.55A5.981 5.981 0 006.75 15.75v-1.5", type: "primary" as const },
  { icon: "M20.25 14.15v4.25c0 1.094-.787 2.036-1.872 2.18-2.087.277-4.216.42-6.378.42s-4.291-.143-6.378-.42c-1.085-.144-1.872-1.086-1.872-2.18v-4.25m16.5 0a2.18 2.18 0 00.75-1.661V8.706c0-1.081-.768-2.015-1.837-2.175a48.114 48.114 0 00-3.413-.387m4.5 8.006c-.194.165-.42.295-.673.38A23.978 23.978 0 0112 15.75c-2.648 0-5.195-.429-7.577-1.22a2.016 2.016 0 01-.673-.38m0 0A2.18 2.18 0 013 12.489V8.706c0-1.081.768-2.015 1.837-2.175a48.111 48.111 0 013.413-.387m7.5 0V5.25A2.25 2.25 0 0013.5 3h-3a2.25 2.25 0 00-2.25 2.25v.894m7.5 0a48.667 48.667 0 00-7.5 0M12 12.75h.008v.008H12v-.008z", type: "gold" as const },
  { icon: "M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z", type: "primary" as const },
  { icon: "M15.59 14.37a6 6 0 01-5.84 7.38v-4.8m5.84-2.58a14.98 14.98 0 006.16-12.12A14.98 14.98 0 009.631 8.41m5.96 5.96a14.926 14.926 0 01-5.841 2.58m-.119-8.54a6 6 0 00-7.381 5.84h4.8m2.581-5.84a14.927 14.927 0 00-2.58 5.84m2.699 2.7c-.103.021-.207.041-.311.06a15.09 15.09 0 01-2.448-2.448 14.9 14.9 0 01.06-.312m-2.24 2.39a4.493 4.493 0 00-1.757 4.306 4.493 4.493 0 004.306-1.758M16.5 9a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0z", type: "gold" as const },
];

export default function BeneficiariesSection() {
  const { t, lang } = useI18n();
  const headerUrl = useSiteImage("header_beneficiaries", headerBeneficiaries);
  const { rows: dbInitiatives } = useDbList<{ id: string; title_ar: string; title_en: string | null; desc_ar: string | null; desc_en: string | null; icon_path: string; accent: string }>("initiatives");

  const initiatives = dbInitiatives.length
    ? dbInitiatives.map((it, i) => ({
        title: lang === "ar" ? it.title_ar : it.title_en || it.title_ar,
        desc: lang === "ar" ? it.desc_ar || "" : it.desc_en || it.desc_ar || "",
        icon: it.icon_path || fallbackInitiatives[i % fallbackInitiatives.length].icon,
        type: (it.accent === "gold" ? "gold" : "primary") as "gold" | "primary",
      }))
    : fallbackInitiatives.map((it, i) => ({
        title: t(`beneficiaries.i${i + 1}Title`),
        desc: t(`beneficiaries.i${i + 1}Desc`),
        icon: it.icon,
        type: it.type,
      }));
  return (
